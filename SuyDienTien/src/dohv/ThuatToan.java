...
public class ThuatToan {
	...
	public void thuatToan(){
		docLuat("D:\\Documents\\luat.txt");
		String giaThiet="a,b,k";
		 String arrGiaThiet[]=giaThiet.split(",");
         GT.addAll(Arrays.asList(arrGiaThiet));
         String ketLuat="m";
         String arrKetLuat[]=ketLuat.split(",");
         KL.addAll(Arrays.asList(arrKetLuat));
		System.out.printf("Tập giả thiết ban đầu: %s",GT);
		System.out.println("");
		System.out.printf("Tập kết luận ban đầu: %s",KL);
		System.out.println("");
	...
	while(kt_KL_Thuoc_TG()==false && SAT.size()!=0) {
			...
			if (!kiemTraTrung(r.vp)) // kiem tra xem vp cua r da thuoc TG chua neu chua thi them vp va TG
			{
				TG.add(r.vp);
			}
